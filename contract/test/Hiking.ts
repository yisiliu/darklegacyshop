...
describe("HikingChallenge", function () {
  ...
  beforeEach(async function () {
    ...
    hikingChallenge = await HikingChallenge.deploy(usdcToken.address, rewardPool.address);
  ...
  describe("Route Creation", function () {
    it("Should create a new route", async function () {
      const locations = [
      ...
      await expect(hikingChallenge.connect(user1).createRoute(locations, blockLimits, publicKeys, 0))
        .to.emit(hikingChallenge, "RouteCreated")
        .withArgs(0, user1.address, 0);

      const route = await hikingChallenge.routes(0);
      expect(route.creator).to.equal(user1.address);
      expect(route.difficulty).to.equal(0);
      expect(route.isActive).to.be.true;
    });

    it("Should fail to create a route with invalid input", async function () {
      const locations = [{ latitude: 42775200000, longitude: 141692000000 }];
      const blockLimits = [100];
      const publicKeys = [await user1.getAddress()];

      await expect(hikingChallenge.connect(user1).createRoute(locations, blockLimits, publicKeys, 0))
        .to.be.revertedWith("Route must have at least 2 points");
    });
  });
  ...
  describe("Challenge Start", function () {
    beforeEach(async function () {
      ...
      await hikingChallenge.connect(user1).createRoute(locations, blockLimits, publicKeys, 0);
    });

    it("Should start a new challenge", async function () {
      await usdcToken.connect(user2).approve(hikingChallenge.address, ethers.utils.parseUnits("1", 6));

      await expect(hikingChallenge.connect(user2).startChallenge(0))
        .to.emit(hikingChallenge, "ChallengeStarted")
        .withArgs(user2.address, ethers.constants.HashZero, 0, ethers.utils.parseUnits("1", 6));

      const challenge = await hikingChallenge.userChallenge(user2.address);
      expect(challenge.isActive).to.be.true;
      expect(challenge.routeId).to.equal(0);
    });

    it("Should fail to start a challenge with insufficient USDC", async function () {
      await usdcToken.connect(user2).approve(hikingChallenge.address, ethers.utils.parseUnits("0.5", 6));
