...
export default function KeyInventoryCard() {
  ...
  return (
    <Card className="md:col-span-3 bg-gray-800 border-amber-500 text-amber-300">
      <CardHeader>
        <CardTitle className="text-2xl flex items-center">
          <Key className="mr-2 h-6 w-6 text-amber-500" />
          Key Inventory
        </CardTitle>
          ...
          {keys.map((key, index) => (
            <Link key={index} href={key.href} className="block">
              <div className="relative group cursor-pointer">
                ...
                ></div>
                <div className="relative p-4 bg-gray-800 rounded-lg border border-amber-500 overflow-hidden group-hover:border-amber-400 transition-colors duration-300">
                  ...
                  <div className="absolute -bottom-1 -right-1 w-12 h-12 bg-amber-500 rounded-tl-full opacity-30 group-hover:opacity-50 transition-opacity duration-300"></div>
                </div>
              </div>
            </Link>
          ))}
